...
public class ScDeviceAdd extends Activity implements OnClickListener {
	...
	private void initListView() {
		...
		SimpleAdapter listItemAdapter = new SimpleAdapter(this, listItem,
				R.layout.da_list_items, new String[] { "image", "title" },
		...
		lv.setOnItemClickListener(new OnItemClickListener() {
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				if (preIdx == -1) {
					...
					ImageView curImg = (ImageView) arg1
							.findViewById(R.id.da_item_image);
					...
					if (preIdx != arg2) {
						...
						ImageView preImg = (ImageView) arg0.getChildAt(preIdx)
								.findViewById(R.id.da_item_image);
	...
	/*
	 * // radio button private void setRadioGroupChangeListener(){
	 * rg_type.setOnCheckedChangeListener(new DevTypeOnCheckedChangeListener());
	 * }
	 * 
	 * private class DevTypeOnCheckedChangeListener implements
	 * OnCheckedChangeListener{ public void onCheckedChanged(RadioGroup group,
	 * int checkedId){ if(rb_tv.getId()==checkedId){
	 * sDevType=rb_tv.getText().toString(); } else
	 * if(rb_air.getId()==checkedId){ sDevType = rb_air.getText().toString(); }
	 * } }
	 */
	...
	public void onClick(View v) {
		if (v.getId() == R.id.da_save_btn) {
			...
			if (devName.equals("")) {
				...
				Intent in = getIntent();
				in.putExtra("new_dev_result", sDevType + ":" + devName); // result
																			// format:
																			// type:name
	 ...
	 * 
	 * @Override public void onFinish() { wm.removeView(floatbtn); }
	 * 
